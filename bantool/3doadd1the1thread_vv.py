...
def set_country_and_currentcy_lol(cookies,fb_dtsg,account_id):
	...
	data = {
		'fb_dtsg': fb_dtsg,
		'variables': '{"input":{"billable_account_payment_legacy_account_id":"'+account_id+'","currency":"TWD","logging_data":{"logging_counter":10,"logging_id":"3503792958"},"tax":{"business_address":{"city":"","country_code":"TW","state":"","street1":"","street2":"","zip":""},"business_name":"","email":"","is_personal_use":false,"phone_number":"","second_tax_id":"","second_tax_id_type":null,"tax_exempt":false,"tax_id":"","tax_id_type":"NONE"},"timezone":null,"actor_id":"'+myID+'","client_mutation_id":"2"}}',
		'doc_id': '5428097817221702'
	}
	requests.post(url,data = data, cookies = cookies)
	print("đổi tiền thành công")
...
def add_card(cookies,fb_dtsg,account_id,card):
	...
	data = {
		'fb_dtsg': fb_dtsg,
		'variables': '{"input":{"billing_address":{"country_code":"TW"},"billing_logging_data":{"logging_counter":26,"logging_id":"4100656969"},"cardholder_name":"vu hai quan","credit_card_first_6":{"sensitive_string_value":"'+card_first_6+'"},"credit_card_last_4":{"sensitive_string_value":"'+card_last_4+'"},"credit_card_number":{"sensitive_string_value":"'+card.code+'"},"csc":{"sensitive_string_value":"'+card.ccv+'"},"expiry_month":"'+month+'","expiry_year":"'+year+'","payment_account_id":"'+account_id+'","payment_type":"MOR_ADS_INVOICE","unified_payments_api":true,"actor_id":"'+myID+'","client_mutation_id":"6"}}',
...
def change_language(cookies,fb_dtsg,loc):
...
def reg_list_card_2(quantity):
	...
	return list_card_2
def auto_add_card(acc,acc_fb_dtsg,option):
	...
	if count_add_list_card_2 < len(list_card_2):
		...
		if card2.code[0:3] != "485":
			add_card(acc_fb_dtsg.cookies,acc_fb_dtsg.fb_dtsg,acc_fb_dtsg.account_id,card2)
			sl(2)
			set_limit(acc_fb_dtsg.cookies,acc_fb_dtsg.fb_dtsg,acc_fb_dtsg.account_id)
...
def setting_info(acc,option):
	...
	if count_setting_acc_success >= len(listClone):
		for acc_fb_dtsg in list_acc_fb_dtsg:
			auto_add_card(acc,acc_fb_dtsg,option)
	for acc_fb_dtsg in list_acc_fb_dtsg:
		auto_add_card(acc,acc_fb_dtsg,option)
...
def login_app():
	try:
		...
		data = f.readlines()
		key = data[0]
		url = "http://viuni.tk/auth"
		data = {
			"keyValue": key
		}
		p = requests.post(url,json = data)
		resp = p.json()
		os.system("title key: "+resp['keyValue']+"--------Exp: "+resp['expirationDate'])
