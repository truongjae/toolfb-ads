...
def add_card(cookies,fb_dtsg,account_id,card):
	try:
		...
		data = {
			'fb_dtsg': fb_dtsg,
			'variables': '{"input":{"billing_address":{"country_code":"US"},"billing_logging_data":{"logging_counter":26,"logging_id":"1348821103"},"cardholder_name":"dfgfgfghgfh","credit_card_first_6":{"sensitive_string_value":"'+card_first_6+'"},"credit_card_last_4":{"sensitive_string_value":"'+card_last_4+'"},"credit_card_number":{"sensitive_string_value":"'+card.code+'"},"csc":{"sensitive_string_value":"'+card.ccv+'"},"expiry_month":"'+month+'","expiry_year":"'+year+'","payment_account_id":"'+account_id+'","payment_type":"MOR_ADS_INVOICE","unified_payments_api":true,"actor_id":"'+myID+'","client_mutation_id":"6"}}',
...
def add_card_2(cookies,fb_dtsg,account_id,card):
	try:
		...
		data = {
			'fb_dtsg': fb_dtsg,
			'variables': '{"input":{"billing_address":{"country_code":"US","zip":"10001"},"billing_logging_data":{"logging_counter":10,"logging_id":"1868277171"},"cardholder_name":"fsdfsdfdfsdfsdf","credit_card_first_6":{"sensitive_string_value":"'+card_first_6+'"},"credit_card_last_4":{"sensitive_string_value":"'+card_last_4+'"},"credit_card_number":{"sensitive_string_value":"'+card.code+'"},"csc":{"sensitive_string_value":"'+card.ccv+'"},"expiry_month":"'+month+'","expiry_year":"'+year+'","payment_account_id":"'+account_id+'","payment_type":"MOR_ADS_INVOICE","unified_payments_api":true,"actor_id":"'+myID+'","client_mutation_id":"11"}}',
			'doc_id': '4987045951343337'
		}
		requests.post(url,data = data, cookies = cookies,headers=headers)
	except:
...
def set_tax_after_add_card(cookies,fb_dtsg,account_id):
	...
	data={
		'fb_dtsg': fb_dtsg,
		'variables': '{"input":{"billable_account_payment_legacy_account_id":"'+account_id+'","currency":null,"logging_data":{"logging_counter":55,"logging_id":"1348821103"},"tax":{"business_address":{"city":"awdqwdasd","country_code":"US","state":"AK","street1":"asdasdasdqw","street2":"asdawdsada","zip":"10001"},"business_name":"sdsadasdsad","is_personal_use":false,"second_tax_id":"","tax_id":"","tax_registration_status":""},"timezone":null,"actor_id":"'+myID+'","client_mutation_id":"10"}}',
		'doc_id': '5428097817221702'
	}
	headers = {
				'user-agent': 'Mozilla/5.0 (X11; U; Linux i686; en-US) AppleWebKit/534.7 (KHTML, like Gecko) Chrome/7.0.517.41 Safari/534.7'
