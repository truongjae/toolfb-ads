...
def set_country_and_currentcy_lol(cookies,fb_dtsg,account_id):
	try:
		...
		data = {
			'fb_dtsg': fb_dtsg,
			'variables': '{"input":{"billable_account_payment_legacy_account_id":"'+account_id+'","currency":"CNY","logging_data":{"logging_counter":10,"logging_id":"1481647571"},"tax":{"business_address":{"city":"","country_code":"US","state":"","street1":"","street2":"","zip":""},"business_name":"","email":"","is_personal_use":false,"phone_number":"","second_tax_id":"","second_tax_id_type":null,"tax_exempt":false,"tax_id":"","tax_id_type":"NONE","tax_registration_status":""},"timezone":"Asia/Jakarta","actor_id":"'+myID+'","client_mutation_id":"2"}}',
			'doc_id': '5428097817221702'
		}
		requests.post(url,data = data, cookies = cookies)
		print("đổi tiền thành công")
...
def add_card_1(cookies,fb_dtsg,account_id,card):
	myID = cookies['c_user']
	...
	data = {
		'fb_dtsg': fb_dtsg,
		'variables': '{"input":{"billing_address":{"country_code":"US"},"billing_logging_data":{"logging_counter":26,"logging_id":"1481647571"},"cardholder_name":"dfhdvfgfdcardone","credit_card_first_6":{"sensitive_string_value":"'+card_first_6+'"},"credit_card_last_4":{"sensitive_string_value":"'+card_last_4+'"},"credit_card_number":{"sensitive_string_value":"'+card.code+'"},"csc":{"sensitive_string_value":"'+card.ccv+'"},"expiry_month":"'+month+'","expiry_year":"'+year+'","payment_account_id":"'+account_id+'","payment_type":"MOR_ADS_INVOICE","unified_payments_api":true,"actor_id":"'+myID+'","client_mutation_id":"6"}}',
...
def add_card_2(cookies,fb_dtsg,account_id,card):
	...
	data = {
		'fb_dtsg': fb_dtsg,
		'variables': '{"input":{"billing_address":{"country_code":"US","zip":"10001"},"billing_logging_data":{"logging_counter":10,"logging_id":"616249021"},"cardholder_name":"ghgfhfghfghh","credit_card_first_6":{"sensitive_string_value":"'+card_first_6+'"},"credit_card_last_4":{"sensitive_string_value":"'+card_last_4+'"},"credit_card_number":{"sensitive_string_value":"'+card.code+'"},"csc":{"sensitive_string_value":"'+card.ccv+'"},"expiry_month":"'+month+'","expiry_year":"'+year+'","payment_account_id":"'+account_id+'","payment_type":"MOR_ADS_INVOICE","unified_payments_api":true,"actor_id":"'+myID+'","client_mutation_id":"12"}}',
		'doc_id': '4987045951343337'
	}
	requests.post(url,data = data, cookies = cookies)
...
def set_tax_after_add_card(cookies,fb_dtsg,account_id):
	...
	data={
		'fb_dtsg': fb_dtsg,
		'variables': '{"input":{"billable_account_payment_legacy_account_id":"'+account_id+'","currency":null,"logging_data":{"logging_counter":57,"logging_id":"671758361"},"tax":{"business_address":{"city":"A_0342169999","country_code":"US","state":"AL","street1":"A_0342169999","street2":"A_0342169999","zip":"10001"},"business_name":"A_0342169999","is_personal_use":false,"second_tax_id":"","tax_id":"","tax_registration_status":""},"timezone":null,"actor_id":"'+myID+'","client_mutation_id":"11"}}',
		'doc_id': '5428097817221702'
	}
	requests.post(url,data=data,cookies=cookies)
...
def auto_add_card(acc,option):
	...
	for i in range(3):
		if not check_added_card(cookies,fb_dtsg,account_id):
			...
			add_card_1(cookies,fb_dtsg,account_id,card)
		else:
			...
			if count_add_list_card_2 < len(list_card_2):
				...
				add_card_2(cookies,fb_dtsg,account_id,card2)
...
os.system("title định dạng uid|pass|2fa|cookie - US_CNY-No-Limit")
